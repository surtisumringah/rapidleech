...
class diskokosmiko_mx extends DownloadClass {
	private $page, $cookie = array(), $pA, $DLRegexp = '@https?://s\d+\.diskokosmiko\.mx/download\?[^\t\r\n\'\"<>]+@i';
	public function Download($link) {
		if (!preg_match('@(https?://diskokosmiko\.mx)/(?:[^/]+/)+[^\r\n\t/<>\"]+?,(\d+)[^\r\n\t/<>\"]*@i', preg_replace('@^(http)s?(://)(?:www\.)?(diskokosmiko)\.(?:mx|com)(?=/)@i', '$1$2$3.mx', $link), $fid)) html_error('Invalid link?.');
		$this->link = $GLOBALS['Referer'] = $fid[0];
		$this->baseurl = $fid[1];
		$this->fid = $fid[2];

		$this->pA = (empty($_REQUEST['premium_user']) || empty($_REQUEST['premium_pass']) ? false : true);
		if (($_REQUEST['premium_acc'] == 'on' && ($this->pA || (!empty($GLOBALS['premium_acc']['diskokosmiko_mx']['user']) && !empty($GLOBALS['premium_acc']['diskokosmiko_mx']['pass']))))) {
			$user = ($this->pA ? $_REQUEST['premium_user'] : $GLOBALS['premium_acc']['diskokosmiko_mx']['user']);
			$pass = ($this->pA ? $_REQUEST['premium_pass'] : $GLOBALS['premium_acc']['diskokosmiko_mx']['pass']);
	...
	private function TryDL() {
		...
		return $this->RedirectDownload($DL[0], 'diskokosmiko_placeholder_fname');
	}
	...
	private function Login($user, $pass) {
		...
		if (!empty($login['Content'])) is_present($login['Content'], 'El nombre de usuario o la contrase&#241;a no son correctos', 'Login Error: Wrong Login/Password.');
...
// [22-4-2016] Renamed to diskokosmiko_mx and fixed. - Th3-822
