...
class cloudtime_to extends DownloadClass {
	public function Download($link) {
		...
		if (!preg_match('@<div class="video_det">\s*<strong>\s*([^"<>]+)\s*</strong>@i', $page, $title)) html_error('Error: Video title not found.');
		...
		$filename .= sprintf(' [CloudTime][%s]%s', $matches[2], $ext[0]);
